...
def click_get_config_file(conf_file):
    ...
    if os.path.isfile(conf_file):
        ...
        click.echo(
            click.style("Parameter is not a configuration file. See --help.", fg="red")
        )
        time.sleep(3)
...
@click.option(
    "--connect",
    prompt="Instance to connect",
    type=click.INT,
    help="Select started instance",
)
@click.argument("conf_file")
def handle_connect(connect, conf_file):
    if connect < 0 or connect > global_state.ec2_instance_count:
        ...
        global_state.df_ec2_attributes["InstanceId"][connect]
        not in global_state.running_instances
    ):
        click.echo(
            click.style(
                f"The selected line {connect} is not in a running state", fg="red"
            )
        )
    else:
        ...
        if sys.platform.startswith("win"):
            if global_state.df_ec2_attributes["Platform"][connect] == "windows":
                ...
                if global_state.config_data["winssh"] == "WSL":
                    cmd = (
                        f'start cmd.exe /K bash ~ -c "ssh {os_user}@{ip} -i {pem_file}"'
                    )
...
def handle_start(start, conf_file):
    if start < 0 or start > global_state.ec2_instance_count:
        ...
        global_state.df_ec2_attributes["InstanceId"][start]
        not in global_state.stopped_instances
    ):
        click.echo(
            click.style(
                f"The selected line {start} is not in a stopped state", fg="red"
            )
        )
        ...
        get_client(
            "ec2",
            global_state.config_data["aws_access_key_id"],
            global_state.config_data["aws_secret_access_key"],
            global_state.config_data["region"],
        ).start_instances(
...
@click.command()
@click.option(
    "--stop", prompt="Instance to stop", type=click.INT, help="Select started instance"
)
@click.argument("conf_file")
def handle_stop(stop, conf_file):
    if stop < 0 or stop > global_state.ec2_instance_count:
        ...
        global_state.df_ec2_attributes["InstanceId"][stop]
        not in global_state.running_instances
    ):
        click.echo(
            click.style(f"The selected line {stop} is not in a running state", fg="red")
        )
...
def get_cpu_metrics_for_instances():
    ...
    while True:
        for instance in global_state.running_instances:
            ...
            if cpu_for_instance:
                global_state.cpu_for_instance.update(
                    {instance: cpu_for_instance[-1]["Maximum"]}
                )
