...
class GlobalState:
    ...
    def set_route53_records(self):
        ...
        try:
            all_dns_records = pd.DataFrame(
                get_client(
                    "route53",
                    self.config_data["aws_access_key_id"],
    ...
    def set_ec2_attributes(self):
        ...
        non_terminated_instances = [
            x for y in instances for x in y if x["State"]["Code"] != 48
        ]
        ...
        for cntr, instance in enumerate(non_terminated_instances):
            attributes = {
                ...
                "LocalLaunchTime": arrow.get(instance["LaunchTime"]).to(
                    self.config_data["timezone"]
                ),
                "PrivateIpAddress": instance["PrivateIpAddress"],
                "PublicIp": instance["NetworkInterfaces"][0]
                .get("Association", {"PublicIp": "-"})
                .get("PublicIp"),
                "FQDN": "-",
            ...
            if self.config_data.get("HostedZoneId") is not None:
                fqdn = self.a_dns_records[
                    self.a_dns_records["IP"] == instance["PrivateIpAddress"]
                ].to_dict("records")
                if fqdn:
            ...
            for tag in tags:
                ...
                if (
                    tag["Key"] == "aws:elasticmapreduce:instance-group-role"
                    and tag["Value"] == "MASTER"
                ):
                    attributes["EMRNodeType"] = "M"

            if instance["State"]["Name"] == "running":
                self.running_instances.append(instance["InstanceId"])
                attributes["Uptime"] = arrow.get(
                    attributes["LocalLaunchTime"]
